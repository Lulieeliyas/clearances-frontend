...
export default function CampusPolicePage() {
  ...
  // ================= CAMPUS POLICE SPECIFIC STATE =================
  const [policeRegistry, setPoliceRegistry] = useState([]);
  ...
  useEffect(() => {
    ...
    if (parsed.role !== "campuspolice") {
      message.error("Access denied. Campus Police only.");
      navigate("/login");
      return;
    }
  ...
  const loadPendingPaymentsCount = async () => {
    ...
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        const policePayments = res.data.filter(payment => 
          payment.department_type === 'campuspolice'
        );
        setPendingPaymentsCount(policePayments.length);
      }
    } catch (err) {
  ...
  const loadStudentsForChat = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}chat/campuspolice/students/`, {
        headers: { Authorization: `Token ${token}` }
      });
  ...
  const loadChatMessages = async (roomId) => {
    try {
      setLoadingChat(true);
      const response = await axios.get(`${API_BASE}chat/campuspolice/messages/${roomId}/`, {
        headers: { Authorization: `Token ${token}` }
      });
  ...
  const startChatWithStudent = async (studentId) => {
    try {
      const response = await axios.post(`${API_BASE}chat/campuspolice/start/`, 
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE}chat/campuspolice/send/`, {
  ...
  const toggleSelectAll = () => {
    if (selectAll) {
      ...
      const pendingForms = getFilteredByCriteria()
        .filter(f => f.status === "approved_sportmaster")
        .map(f => f.id);
  ...
  const checkStudentPoliceIssues = (studentId) => {
    return policeRegistry.some(record => 
      record.student_id === studentId && 
      record.status === 'pending'
    );
  };

  const getStudentPoliceIssues = (studentId) => {
    return policeRegistry.filter(record => record.student_id === studentId);
  };

  const getTotalStudentFines = (studentId) => {
    return policeRegistry
      .filter(record => record.student_id === studentId && record.status === 'pending')
      .reduce((total, record) => total + (record.fine_amount || 0), 0);
  };
    ...
    try {
      ...
      for (let i = 0; i < selectedForms.length; i++) {
        ...
        try {
          ...
          // Check if student has police issues
          const hasIssues = checkStudentPoliceIssues(form.id_number);
          const studentFines = getTotalStudentFines(form.id_number);
          
          if (hasIssues && batchAction === 'approve') {
            results.failed.push({
              id: formId,
              name: form.full_name,
              reason: `Student has pending police issues (Fines: $${studentFines})`
            });
            continue;
          }
          ...
          const payload = {
            action: batchAction,
            note: batchNotes || `Batch ${batchAction} by Campus Police`
          };
          
          if (batchAction === 'reject' && batchPaymentRequired) {
            payload.requires_payment = true;
            payload.payment_amount = parseFloat(batchPaymentAmount) + studentFines;
            payload.payment_reason = batchPaymentReason || 'Police fines required';
            totalFinesAmount += parseFloat(batchPaymentAmount) + studentFines;
          }
          
          const res = await axios.patch(
            `${API_BASE}campuspolice/action/${formId}/`,
            payload,
            { headers: { Authorization: `Token ${token}` } }
          );
          ...
          setForms(prev => prev.map(f => {
            if (f.id === formId) {
              const newStatus = batchAction === 'approve' ? 'approved_campuspolice' : 'rejected';
              return { 
                ...f, 
                status: newStatus,
                campuspolice_note: batchNotes || f.campuspolice_note
              };
            }
            return f;
          }));
      ...
      notification.info({
        ...
        description: (
          <div>
            ...
            {totalFinesAmount > 0 && (
              <p><DollarOutlined /> Total Fines: ${totalFinesAmount.toFixed(2)}</p>
            )}
          </div>
        ),
        duration: 5
  ...
  const getFilteredByCriteria = () => {
    let filtered = forms.filter(f => f.status === "approved_sportmaster");
    
    if (filterCriteria.department) {
      filtered = filtered.filter(f => f.department_name === filterCriteria.department);
    }
    
    if (filterCriteria.incidentType) {
      filtered = filtered.filter(f => {
        const issues = getStudentPoliceIssues(f.id_number);
        return issues.some(i => i.incident_type === filterCriteria.incidentType);
      });
    }
    
    if (filterCriteria.hasFines) {
      filtered = filtered.filter(f => getTotalStudentFines(f.id_number) > 0);
    }
  ...
  const getUniqueIncidentTypes = () => {
    const types = new Set(policeRegistry.map(r => r.incident_type).filter(Boolean));
    ...
    <Menu
      onClick={({ key }) => {
        if (key === "payment-verification") {
          sessionStorage.setItem("payment_verification_dept", "campuspolice");
          navigate("/staff/payments");
        }
        if (key === "clearance-forms") {
          loadForms(token);
        }
        if (key === "police-registry") {
      ...
      items={[
        ...
        },
        {
          ...
          label: (
            <Space>
              <DatabaseOutlined />
              <span>Police Registry</span>
              {policeRegistry.length > 0 && (
                <Badge count={policeRegistry.length} size="small" />
              )}
            </Space>
          ),
        },
  ...
  const loadForms = async (authToken) => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE}campuspolice/forms/`, {
  ...
  const loadPoliceRegistry = async () => {
    try {
      setLoadingRegistry(true);
      const savedRegistry = localStorage.getItem("campuspolice_registry");
      if (savedRegistry) {
        setPoliceRegistry(JSON.parse(savedRegistry));
      } else {
        setPoliceRegistry([]);
      }
    } catch (err) {
  ...
  const savePoliceRegistry = (registry) => {
    localStorage.setItem("campuspolice_registry", JSON.stringify(registry));
    setPoliceRegistry(registry);
  };
  ...
  const registerNewRecord = async (values) => {
    try {
      const newRecord = {
        ...
        registered_by: user?.username || 'campuspolice'
      };

      const updatedRegistry = [...policeRegistry, newRecord];
      savePoliceRegistry(updatedRegistry);
      
      message.success("Police record registered successfully!");
  ...
  const updateRecord = async (values) => {
    try {
      ...
      const updatedRegistry = policeRegistry.map(record => 
  ...
  const deleteRecord = (id) => {
    Modal.confirm({
      ...
      cancelText: 'No',
      onOk: () => {
        const updatedRegistry = policeRegistry.filter(record => record.id !== id);
        savePoliceRegistry(updatedRegistry);
  ...
  const searchRegistry = () => {
    ...
    return policeRegistry.filter(record => 
  ...
  const approveForm = async (formId, studentId) => {
    try {
      ...
      confirm({
        title: 'Confirm Campus Police Clearance',
        content: (
          <div>
            <p>Have you verified this student has no security or disciplinary issues?</p>
            <Alert
              message="Important:"
              description="Check if student has any pending cases, fines, or disciplinary actions."
              type="warning"
              showIcon
              style={{ marginTop: 10 }}
            />
        ...
        onOk: async () => {
          try {
            const note = "Approved by Campus Police - No pending issues";
            const res = await axios.patch(
              `${API_BASE}campuspolice/action/${formId}/`,
            ...
            setForms(prev => prev.map(f => 
              f.id === formId ? { ...f, status: "approved_campuspolice", campuspolice_note: res.data.note } : f
            ));
  ...
  const handleRejectForm = async () => {
    try {
      ...
      const res = await axios.patch(
        `${API_BASE}campuspolice/action/${selectedFormId}/`,
        payload,
        { headers: { Authorization: `Token ${token}` } }
      );
      ...
      setForms(prev => prev.map(f => {
        if (f.id === selectedFormId) {
          return { 
            ...
            campuspolice_note: res.data.note || rejectNote.trim()
          };
        }
        return f;
      }));
      ...
      if (rejectAction === 'require_payment') {
        notification.info({
          ...
          description: 'Student will receive payment instructions for fines.',
        });
      } else {
        notification.error({
  ...
  const calculateStats = (formsList) => {
    const totalFines = formsList.reduce((total, form) => {
      return total + getStudentTotalFines(form.id_number);
    }, 0);
    
    const stats = {
      total: formsList.length,
      pending: formsList.filter(f => f.status === "approved_sportmaster").length,
      approved: formsList.filter(f => f.status === "approved_campuspolice").length,
      rejected: formsList.filter(f => f.status === "rejected" && f.campuspolice_note).length,
          ...
          <Badge 
            ...
            text={
              <Space>
                ...
                <Tag color="orange">Campus Police Review</Tag>
              </Space>
            }
          />
        );
  ...
  const renderClearanceFlow = (form) => {
    const currentStep = form.status === "approved_sportmaster" ? 5 : 
                      form.status === "approved_campuspolice" ? 6 : 0;
    ...
    return (
      <Steps size="small" current={currentStep} style={{ marginTop: 20 }}>
        ...
        <Step 
          title="Campus Police" 
          description={
            form.status === "approved_campuspolice" ? "Approved" : 
            form.status === "rejected" ? "Rejected" : "Review Needed"
          } 
          icon={
            form.status === "approved_campuspolice" ? <CheckCircleOutlined /> : 
            form.status === "rejected" ? <CloseCircleOutlined /> : <SecurityScanOutlined />
          }
        />
        <Step title="Cooperation Sharing" description="Waiting" icon={<TeamOutlined />} />
      </Steps>
    );
  };
  ...
  const registryColumns = [
    ...
    },
    {
      ...
      key: 'fine_amount',
      render: (amount) => amount > 0 ? <Text type="danger"><DollarOutlined /> {amount}</Text> : 'No fine',
    },
    {
      title: 'Status',
      dataIndex: 'status',
  ...
  const renderFormCard = (form) => {
    const isPending = form.status === "approved_sportmaster";
    const isApproved = form.status === "approved_campuspolice";
    const isRejected = form.status === "rejected";
    
    const hasIssue = checkIfStudentHasIssue(form.id_number);
    const studentIssues = getStudentIssues(form.id_number);
    const totalFines = getStudentTotalFines(form.id_number);
    ...
    return (
      <Card
        ...
        style={{
          ...
          borderLeft: `5px solid ${
            isApproved ? "#52c41a" :
            isRejected ? "#ff4d4f" :
            hasIssue ? "#faad14" : "#fa541c"
          }`,
          transition: 'all 0.3s',
          cursor: 'pointer',
          background: hasIssue ? '#fff2e8' : 'white',
        ...
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Space>
              ...
              {hasIssue && (
                <Tooltip title={`Student has ${studentIssues.length} pending police issue(s) - Total Fines: $${totalFines}`}>
      ...
      >
        ...
        {hasIssue && (
          <Alert
            message="Pending Police Issues"
            description={
              <div>
                <p>Student has {studentIssues.length} pending police issue(s):</p>
                <ul>
                  {studentIssues.map(issue => (
                    <li key={issue.id}>
                      {issue.incident_type}: {issue.description} 
                      {issue.fine_amount > 0 && <Tag color="red" style={{ marginLeft: 8 }}>Fine: ${issue.fine_amount}</Tag>}
                    </li>
                  ))}
                </ul>
                {totalFines > 0 && (
                  <Text type="danger"><DollarOutlined /> Total Outstanding Fines: ${totalFines}</Text>
                )}
              </div>
            }
            type="warning"
        ...
        {form.sportmaster_note && (
          <Alert message="Sport Master Note" description={form.sportmaster_note} type="info" showIcon style={{ marginTop: 15 }} />
        )}
        
        {isPending && !batchMode && (
          <Space style={{ marginTop: 15, width: '100%', justifyContent: 'center' }}>
            <Popconfirm
              title="Verify Campus Police Status"
              description="Have you checked all security and disciplinary records for this student?"
        ...
        {form.campuspolice_note && (
          <Alert
            message="Campus Police Action"
            description={form.campuspolice_note}
    ...
    <div style={{ padding: 30, maxWidth: 1400, margin: '0 auto', minHeight: '100vh', background: '#f0f2f5' }}>
      ...
      <Card 
        style={{ 
          marginBottom: 30,
          borderRadius: 15,
          background: 'linear-gradient(135deg, #fa541c 0%, #d4380b 100%)',
          color: 'white',
          boxShadow: '0 8px 25px rgba(250, 84, 28, 0.3)'
        }}
          ...
          <Col>
            <Space direction="vertical" size={0}>
              <Title level={2} style={{ color: 'white', margin: 0 }}>
                <SecurityScanOutlined /> Campus Police Clearance
              </Title>
              <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px' }}>
                Welcome, {user?.username || 'Campus Police Officer'}
              </Text>
            </Space>
          </Col>
          <Col>
            <Space size="middle">
              ...
              >
                <Space>
                  ...
                  <span>Police Registry</span>
                  {policeRegistry.length > 0 && (
                    <Badge count={policeRegistry.length} size="small" style={{ backgroundColor: '#ff4d4f' }} />
                  )}
                </Space>
              </Button>
      ...
      <Row gutter={16} style={{ marginBottom: 30 }}>
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #fa541c' }}>
            <Statistic title="Total Forms" value={stats.total} valueStyle={{ color: '#fa541c' }} prefix={<FileTextOutlined />} />
            <Text type="secondary">From Sport Master</Text>
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #1890ff' }}>
            ...
            <Text type="secondary">Need police check</Text>
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
        ...
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #faad14' }}>
            ...
            <Text type="secondary">Pending police issues</Text>
            {stats.totalFines > 0 && (
              <div><Tag color="red">Total Fines: ${stats.totalFines}</Tag></div>
            )}
          </Card>
        </Col>
      </Row>
      ...
      <Tabs defaultActiveKey="1" style={{ marginBottom: 20 }}>
        <TabPane 
          tab={
            <span>
              <SecurityScanOutlined /> Clearance Forms 
        ...
        >
          ...
          <Card style={{ marginBottom: 20, borderRadius: 10, background: '#fff2e8' }}>
                    ...
                    overlay={
                      <Menu>
                        ...
                        <Menu.Item key="incident">
                          <Space><WarningOutlined /> Filter by Incident Type</Space>
                        </Menu.Item>
                        <Menu.Item key="fines" onClick={() => setFilterCriteria(prev => ({ ...prev, hasFines: !prev.hasFines }))}>
                          <Space><DollarOutlined /> {filterCriteria.hasFines ? "Show All" : "Show Only With Fines"}</Space>
                        </Menu.Item>
                      </Menu>
                    }
                  >
            ...
            {Object.values(filterCriteria).some(v => v) && (
              <div style={{ marginTop: 15 }}>
                <Space wrap>
                  ...
                  {filterCriteria.incidentType && (
                    <Tag closable onClose={() => setFilterCriteria(prev => ({ ...prev, incidentType: '' }))}>
                      Incident: {filterCriteria.incidentType}
                    </Tag>
                  )}
                  {filterCriteria.hasFines && (
                    <Tag closable onClose={() => setFilterCriteria(prev => ({ ...prev, hasFines: false }))}>
                      With Fines Only
                    </Tag>
                  )}
          ...
          <Alert
            message="CAMPUS POLICE CLEARANCE REQUIRED"
            description={
              <Space direction="vertical" size={8}>
                <Paragraph>
                  <ol>
                    <li>Check disciplinary records for any pending cases</li>
                    <li>Verify if student has any outstanding fines</li>
                    <li>Check security incidents or violations</li>
                    <li>Approve only if all police requirements are met</li>
                  </ol>
                </Paragraph>
              </Space>
            }
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              <Spin size="large" />
              <p style={{ marginTop: 20 }}>Loading forms from sport master...</p>
            </div>
          ) : filteredForms.length === 0 ? (
            <Card style={{ textAlign: 'center', padding: 60 }}>
              <FileTextOutlined style={{ fontSize: 64, color: '#d9d9d9' }} />
              <Title level={4} type="secondary">No forms awaiting campus police clearance</Title>
            </Card>
          ) : (
            <>
              {batchMode && (
                  ...
                  chatRooms.map(room => (
                    <Card
                      ...
                      style={{ 
                        marginBottom: '10px', 
                        cursor: 'pointer',
                        background: selectedChatRoom?.id === room.id ? '#fff2e8' : 'white',
                        borderColor: selectedChatRoom?.id === room.id ? '#fa541c' : '#f0f0f0'
                      }}
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        {room.unread_count > 0 && (
                          <Badge count={room.unread_count} style={{ backgroundColor: '#fa541c' }} />
                        )}
                      </div>
                      ...
                      chatMessages.map(msg => (
                        ...
                        >
                          <div style={{
                            ...
                            padding: '10px 15px',
                            borderRadius: '15px',
                            background: msg.is_own ? '#fa541c' : '#f0f0f0',
        ...
        <TabPane tab={<span><DatabaseOutlined /> Police Registry</span>} key="2">
            ...
            <Row gutter={16} align="middle">
              ...
              <Col span={8} style={{ textAlign: 'right' }}>
                <Space>
                  ...
                  <Button icon={<ReloadOutlined />} onClick={loadPoliceRegistry} loading={loadingRegistry}>
                    Refresh
                  </Button>
                </Space>
              </Col>
          ...
          <Table
            ...
            title={() => (
              <Space>
                <DatabaseOutlined />
                <Text strong>Campus Police Registry</Text>
                <Tag color="red">Total Records: {searchRegistry().length}</Tag>
                ...
                <Tag color="gold">
                  Total Fines: ${searchRegistry().reduce((sum, r) => sum + (r.fine_amount || 0), 0)}
                </Tag>
              </Space>
            )}
          />
      ...
      <Modal
        title={
          <Space>
            <RocketOutlined style={{ color: '#fa541c' }} />
              ...
              {batchAction === 'reject' && (
                <>
                  ...
                  <Form.Item name="requirePayment">
                    ...
                    >
                      Require Fine Payment for These Forms
                    </Checkbox>
                  </Form.Item>
                        ...
                        <Col span={12}>
                          ...
                          >
                            <Input
                              ...
                              placeholder="e.g., Security violation fine"
                            />
                          </Form.Item>
                        </Col>
                      </Row>
              ...
              <div style={{ textAlign: 'center' }}>
                <Space size="large">
                  ...
                  <Button
                    ...
                    style={batchAction === 'approve' ? { background: '#fa541c' } : {}}
                ...
                <List
                  ...
                  renderItem={item => (
                    <List.Item>
                      ...
                      {item.fines > 0 && (
                        <Tag color="red" style={{ marginLeft: 8 }}>Fines: ${item.fines}</Tag>
                      )}
                    </List.Item>
                  )}
                />
          ...
          <div style={{ maxHeight: '60vh', overflow: 'auto' }}>
            <Descriptions column={2} bordered size="middle" labelStyle={{ fontWeight: 'bold', background: '#fafafa' }}>
              ...
              <Descriptions.Item label="Reason" span={2}>
                <Card size="small" style={{ background: '#fff2e8' }}>{selectedForm.reason}</Card>
              </Descriptions.Item>
              {selectedForm.sportmaster_note && (
                <Descriptions.Item label="Sport Master Note" span={2}>
                  <Alert message={selectedForm.sportmaster_note} type="info" showIcon />
                </Descriptions.Item>
              )}
            </Descriptions>

            {selectedForm.status === "approved_sportmaster" && (
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <Select style={{ width: '100%', marginTop: 5 }} value={rejectAction} onChange={setRejectAction}>
            ...
            <Option value="require_payment">Require Payment (Fines)</Option>
          </Select>
        </div>
        ...
        {rejectAction === 'require_payment' && (
          <div style={{ marginBottom: 20, padding: 15, background: '#f6f8fa', borderRadius: 8 }}>
            ...
            <Input placeholder="Fine Amount" prefix="$" type="number" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} style={{ marginBottom: 10 }} />
      ...
      <Modal
        title={<Space><DatabaseOutlined /><span>{editingRecord ? 'Edit Police Record' : 'Register New Police Record'}</span></Space>}
        ...
        width={600}
      >
        <Alert message="Campus Police Registry" description="Keep track of student incidents, fines, and disciplinary actions." type="info" showIcon style={{ marginBottom: 20 }} />
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="fine_amount" label="Fine Amount">
                <InputNumber style={{ width: '100%' }} placeholder="Enter fine amount" prefix="$" min={0} step={0.01} />
              </Form.Item>
            </Col>
            <Col span={12}>
      ...
      <div style={{ marginTop: 40, padding: '20px 0', textAlign: 'center', borderTop: '1px solid #f0f0f0' }}>
        <Text type="secondary">
          Campus Police Clearance System • {new Date().getFullYear()} • 
          <span style={{ marginLeft: 8, color: '#fa541c' }}>
            Total Forms: {stats.total} • Pending: {stats.pending} • Records: {policeRegistry.length}
          </span>
        </Text>
      </div>
    </div>
