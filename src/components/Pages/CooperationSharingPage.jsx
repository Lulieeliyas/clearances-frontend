...
console.log(`🌐 Coopration Share Head API Base: ${API_BASE}`);

export default function CooperationSharingPage() {
  ...
  // ================= COOPERATION SHARING SPECIFIC STATE =================
  ...
  useEffect(() => {
    ...
    if (parsed.role !== "cooperationsharing") {
      message.error("Access denied. Cooperation Sharing only.");
      navigate("/login");
      return;
    }
  ...
  const loadPendingPaymentsCount = async () => {
    ...
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        const coopPayments = res.data.filter(payment => 
          payment.department_type === 'cooperationsharing'
        );
        setPendingPaymentsCount(coopPayments.length);
      }
    } catch (err) {
  ...
  const loadStudentsForChat = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}chat/cooperationsharing/students/`, {
        headers: { Authorization: `Token ${token}` }
      });
  ...
  const loadChatMessages = async (roomId) => {
    try {
      setLoadingChat(true);
      const response = await axios.get(`${API_BASE}chat/cooperationsharing/messages/${roomId}/`, {
        headers: { Authorization: `Token ${token}` }
      });
  ...
  const startChatWithStudent = async (studentId) => {
    try {
      const response = await axios.post(`${API_BASE}chat/cooperationsharing/start/`, 
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE}chat/cooperationsharing/send/`, {
  ...
  const toggleSelectAll = () => {
    if (selectAll) {
      ...
      const pendingForms = getFilteredByCriteria()
        .filter(f => f.status === "approved_campuspolice")
        .map(f => f.id);
  ...
  const checkStudentCoopIssues = (studentId) => {
    return coopRegistry.some(record => 
      record.student_id === studentId && 
      record.status === 'pending'
    );
  };

  const getStudentCoopIssues = (studentId) => {
    return coopRegistry.filter(record => record.student_id === studentId);
  };

  const getStudentTotalObligations = (studentId) => {
    return coopRegistry
      .filter(record => record.student_id === studentId && record.status === 'pending')
    ...
    try {
      ...
      for (let i = 0; i < selectedForms.length; i++) {
        ...
        try {
          ...
          // Check if student has cooperation issues
          const hasIssues = checkStudentCoopIssues(form.id_number);
          const studentObligations = getStudentTotalObligations(form.id_number);
          
          if (hasIssues && batchAction === 'approve') {
            results.failed.push({
              id: formId,
              name: form.full_name,
              reason: `Student has pending cooperation obligations (Amount: $${studentObligations})`
            });
            continue;
          }
          ...
          const payload = {
            action: batchAction,
            note: batchNotes || `Batch ${batchAction} by Cooperation Sharing`
          };
          
          if (batchAction === 'reject' && batchPaymentRequired) {
            payload.requires_payment = true;
            payload.payment_amount = parseFloat(batchPaymentAmount) + studentObligations;
            payload.payment_reason = batchPaymentReason || 'Cooperation obligations required';
            totalObligationsAmount += parseFloat(batchPaymentAmount) + studentObligations;
          }
          
          const res = await axios.patch(
            `${API_BASE}cooperationsharing/action/${formId}/`,
            payload,
            { headers: { Authorization: `Token ${token}` } }
          );
          ...
          results.success.push({
            ...
            obligations: studentObligations
          });
          
          // Update local state
          setForms(prev => prev.map(f => {
            if (f.id === formId) {
              const newStatus = batchAction === 'approve' ? 'approved_cooperationsharing' : 'rejected';
              return { 
                ...f, 
                status: newStatus,
                cooperationsharing_note: batchNotes || f.cooperationsharing_note
              };
            }
            return f;
          }));
        ...
        description: (
          <div>
            ...
            {totalObligationsAmount > 0 && (
              <p><DollarOutlined /> Total Obligations: ${totalObligationsAmount.toFixed(2)}</p>
            )}
          </div>
        ),
        duration: 5
  ...
  const getFilteredByCriteria = () => {
    let filtered = forms.filter(f => f.status === "approved_campuspolice");
    
    if (filterCriteria.department) {
      filtered = filtered.filter(f => f.department_name === filterCriteria.department);
    }
    
    if (filterCriteria.cooperationType) {
      filtered = filtered.filter(f => {
        const issues = getStudentCoopIssues(f.id_number);
        return issues.some(i => i.cooperation_type === filterCriteria.cooperationType);
      });
    }
    
    if (filterCriteria.hasObligations) {
      filtered = filtered.filter(f => getStudentTotalObligations(f.id_number) > 0);
    }
  ...
  const getUniqueCoopTypes = () => {
    const types = new Set(coopRegistry.map(r => r.cooperation_type).filter(Boolean));
    ...
    <Menu
      onClick={({ key }) => {
        if (key === "payment-verification") {
          sessionStorage.setItem("payment_verification_dept", "cooperationsharing");
          navigate("/staff/payments");
        }
        if (key === "clearance-forms") {
          loadForms(token);
        }
        if (key === "coop-registry") {
      ...
      items={[
        ...
        },
        {
          ...
          label: (
            <Space>
              <DatabaseOutlined />
              <span>Cooperation Registry</span>
              {coopRegistry.length > 0 && (
                <Badge count={coopRegistry.length} size="small" />
              )}
            </Space>
          ),
        },
  ...
  const loadForms = async (authToken) => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE}cooperationsharing/forms/`, {
  ...
  // ================= LOAD COOPERATION REGISTRY =================
  const loadCoopRegistry = async () => {
    try {
      setLoadingRegistry(true);
      const savedRegistry = localStorage.getItem("cooperationsharing_registry");
      if (savedRegistry) {
        setCoopRegistry(JSON.parse(savedRegistry));
      } else {
        setCoopRegistry([]);
      }
    } catch (err) {
  ...
  const saveCoopRegistry = (registry) => {
    localStorage.setItem("cooperationsharing_registry", JSON.stringify(registry));
    setCoopRegistry(registry);
  };
  ...
  const registerNewRecord = async (values) => {
    try {
      const newRecord = {
        ...
        registered_by: user?.username || 'cooperationsharing'
      };

      const updatedRegistry = [...coopRegistry, newRecord];
      saveCoopRegistry(updatedRegistry);
      
      message.success("Cooperation record registered successfully!");
  ...
  const updateRecord = async (values) => {
    try {
      ...
      const updatedRegistry = coopRegistry.map(record => 
  ...
  const deleteRecord = (id) => {
    Modal.confirm({
      ...
      cancelText: 'No',
      onOk: () => {
        const updatedRegistry = coopRegistry.filter(record => record.id !== id);
        saveCoopRegistry(updatedRegistry);
  ...
  const searchRegistry = () => {
    ...
    return coopRegistry.filter(record => 
  ...
  const approveForm = async (formId, studentId) => {
    try {
      ...
      confirm({
        title: 'Confirm Cooperation Sharing Clearance',
        content: (
          <div>
            <p>Have you verified this student's cooperation sharing obligations?</p>
            <Alert
              message="Important:"
              description="Check if student has any pending cooperation agreements or payments."
              type="warning"
              showIcon
              style={{ marginTop: 10 }}
            />
        ...
        onOk: async () => {
          try {
            const note = "Approved by Cooperation Sharing - No pending issues";
            const res = await axios.patch(
              `${API_BASE}cooperationsharing/action/${formId}/`,
            ...
            setForms(prev => prev.map(f => 
              f.id === formId ? { ...f, status: "approved_cooperationsharing", cooperationsharing_note: res.data.note } : f
            ));
            
            message.success("Form approved success!");
            
            notification.success({
              message: 'Sent success',
              description: 'Clearance form has been approved success!.',
              duration: 5,
            });
  ...
  const handleRejectForm = async () => {
    try {
      ...
      const res = await axios.patch(
        `${API_BASE}cooperationsharing/action/${selectedFormId}/`,
        payload,
        { headers: { Authorization: `Token ${token}` } }
      );
      ...
      setForms(prev => prev.map(f => {
        if (f.id === selectedFormId) {
          return { 
            ...
            cooperationsharing_note: res.data.note || rejectNote.trim()
          };
        }
        return f;
      }));
  ...
  const calculateStats = (formsList) => {
    const totalObligations = formsList.reduce((total, form) => {
      return total + getStudentTotalObligations(form.id_number);
    }, 0);
    
    const stats = {
      total: formsList.length,
      pending: formsList.filter(f => f.status === "approved_campuspolice").length,
      approved: formsList.filter(f => f.status === "approved_cooperationsharing").length,
      rejected: formsList.filter(f => f.status === "rejected" && f.cooperationsharing_note).length,
          ...
          <Badge 
            ...
            text={
              <Space>
                ...
                <Tag color="orange">Cooperation Sharing Review</Tag>
              </Space>
            }
          />
        );
  ...
  const renderClearanceFlow = (form) => {
    const currentStep = form.status === "approved_campuspolice" ? 6 : 
                      form.status === "approved_cooperationsharing" ? 7 : 0;
    ...
    return (
      <Steps size="small" current={currentStep} style={{ marginTop: 20 }}>
        ...
        <Step 
          ...
          description={
            form.status === "approved_cooperationsharing" ? "Approved" : 
            form.status === "rejected" ? "Rejected" : "Review Needed"
          } 
          icon={
            form.status === "approved_cooperationsharing" ? <CheckCircleOutlined /> : 
            form.status === "rejected" ? <CloseCircleOutlined /> : <ShareAltOutlined />
          }
        />
        <Step title="DOP Cordinator" description="Waiting" icon={<UserOutlined />} />
      </Steps>
    );
  };
  ...
  const registryColumns = [
    ...
    },
    {
      ...
      dataIndex: 'amount',
      key: 'amount',
      render: (amount) => amount > 0 ? <Text type="danger"><DollarOutlined /> {amount}</Text> : 'No amount',
    },
    {
      title: 'Due Date',
      dataIndex: 'due_date',
  ...
  const renderFormCard = (form) => {
    const isPending = form.status === "approved_campuspolice";
    const isApproved = form.status === "approved_cooperationsharing";
    const isRejected = form.status === "rejected";
    
    const hasIssue = checkIfStudentHasIssue(form.id_number);
    const studentIssues = getStudentIssues(form.id_number);
    const totalObligations = getStudentTotalObligations(form.id_number);
    ...
    return (
      <Card
        ...
        style={{
          ...
          borderLeft: `5px solid ${
            isApproved ? "#52c41a" :
            isRejected ? "#ff4d4f" :
            hasIssue ? "#faad14" : "#2f54eb"
          }`,
          transition: 'all 0.3s',
          cursor: 'pointer',
          background: hasIssue ? '#f0f5ff' : 'white',
        ...
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Space>
              ...
              {hasIssue && (
                <Tooltip title={`Student has ${studentIssues.length} pending cooperation obligation(s) - Total: $${totalObligations}`}>
      ...
      >
        ...
        {hasIssue && (
          <Alert
            message="Pending Cooperation Obligations"
            description={
              <div>
                <p>Student has {studentIssues.length} pending cooperation obligation(s):</p>
                <ul>
                  {studentIssues.map(issue => (
                    <li key={issue.id}>
                      {issue.cooperation_type}: {issue.description} 
                      {issue.amount > 0 && <Tag color="geekblue" style={{ marginLeft: 8 }}>Amount: ${issue.amount}</Tag>}
                      {issue.due_date && <Tag color="orange">Due: {dayjs(issue.due_date).format('MMM D, YYYY')}</Tag>}
                    </li>
                  ))}
                </ul>
                {totalObligations > 0 && (
                  <Text type="danger"><DollarOutlined /> Total Outstanding Obligations: ${totalObligations}</Text>
                )}
              </div>
            }
            type="warning"
        ...
        {form.campuspolice_note && (
          <Alert message="Campus Police Note" description={form.campuspolice_note} type="info" showIcon style={{ marginTop: 15 }} />
        )}
        
        {isPending && !batchMode && (
          <Space style={{ marginTop: 15, width: '100%', justifyContent: 'center' }}>
            <Popconfirm
              title="Verify Cooperation Sharing Status"
              description="Have you checked all cooperation agreements for this student?"
        ...
        {form.cooperationsharing_note && (
          <Alert
            message="Cooperation Sharing Action"
            description={form.cooperationsharing_note}
    ...
    <div style={{ padding: 30, maxWidth: 1400, margin: '0 auto', minHeight: '100vh', background: '#f0f2f5' }}>
      ...
      <Card 
        style={{ 
          ...
          color: 'white',
          boxShadow: '0 8px 25px rgba(47, 84, 235, 0.3)'
        }}
          ...
          <Col>
            <Space direction="vertical" size={0}>
              <Title level={2} style={{ color: 'white', margin: 0 }}>
                <ShareAltOutlined /> Cooperation Sharing Clearance
              </Title>
              <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px' }}>
                Welcome, {user?.username || 'Cooperation Officer'}
              </Text>
            </Space>
          </Col>
          <Col>
            <Space size="middle">
              ...
              >
                <Space>
                  <DatabaseOutlined />
                  <span>Coop Registry</span>
                  {coopRegistry.length > 0 && (
                    <Badge count={coopRegistry.length} size="small" style={{ backgroundColor: '#ff4d4f' }} />
                  )}
                </Space>
              </Button>
      ...
      <Row gutter={16} style={{ marginBottom: 30 }}>
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #2f54eb' }}>
            <Statistic title="Total Forms" value={stats.total} valueStyle={{ color: '#2f54eb' }} prefix={<FileTextOutlined />} />
            <Text type="secondary">From Campus Police</Text>
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #1890ff' }}>
            ...
            <Text type="secondary">Need cooperation check</Text>
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
        ...
        <Col xs={24} sm={12} md={6}>
          <Card hoverable style={{ textAlign: 'center', borderRadius: 10, border: '2px solid #faad14' }}>
            ...
            <Text type="secondary">Pending obligations</Text>
            {stats.totalObligations > 0 && (
              <div><Tag color="geekblue">Total: ${stats.totalObligations}</Tag></div>
            )}
          </Card>
        </Col>
      </Row>
      ...
      <Tabs defaultActiveKey="1" style={{ marginBottom: 20 }}>
        <TabPane 
          tab={
            <span>
              <ShareAltOutlined /> Clearance Forms 
        ...
        >
                  ...
                  <Dropdown
                    overlay={
                      <Menu>
                        ...
                        <Menu.Item key="coop">
                          <Space><ShareAltOutlined /> Filter by Cooperation Type</Space>
                        </Menu.Item>
                        <Menu.Item key="obligations" onClick={() => setFilterCriteria(prev => ({ ...prev, hasObligations: !prev.hasObligations }))}>
                          <Space><DollarOutlined /> {filterCriteria.hasObligations ? "Show All" : "Show Only With Obligations"}</Space>
                        </Menu.Item>
                      </Menu>
                    }
                  >
            ...
            {Object.values(filterCriteria).some(v => v) && (
              <div style={{ marginTop: 15 }}>
                <Space wrap>
                  ...
                  {filterCriteria.cooperationType && (
                    <Tag closable onClose={() => setFilterCriteria(prev => ({ ...prev, cooperationType: '' }))}>
                      Type: {filterCriteria.cooperationType}
                    </Tag>
                  )}
                  {filterCriteria.hasObligations && (
                    <Tag closable onClose={() => setFilterCriteria(prev => ({ ...prev, hasObligations: false }))}>
                      With Obligations Only
                    </Tag>
                  )}
          ...
          <Alert
            message="COOPERATION SHARING CLEARANCE REQUIRED"
            description={
              <Space direction="vertical" size={8}>
                <Paragraph>
                  <ol>
                    <li>Check cooperation agreements and obligations</li>
                    <li>Verify if student has any pending cooperation payments</li>
                    <li>Review partnership program requirements</li>
                    <li>Approve only if all cooperation requirements are met</li>
                  </ol>
                </Paragraph>
              </Space>
            }
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: 80 }}>
              <Spin size="large" />
              <p style={{ marginTop: 20 }}>Loading forms from campus police...</p>
            </div>
          ) : filteredForms.length === 0 ? (
            <Card style={{ textAlign: 'center', padding: 60 }}>
              <FileTextOutlined style={{ fontSize: 64, color: '#d9d9d9' }} />
              <Title level={4} type="secondary">No forms awaiting cooperation sharing clearance</Title>
            </Card>
          ) : (
            <>
              {batchMode && (
                  ...
                  chatRooms.map(room => (
                    <Card
                      ...
                      style={{ 
                        marginBottom: '10px', 
                        cursor: 'pointer',
                        background: selectedChatRoom?.id === room.id ? '#f0f5ff' : 'white',
                        borderColor: selectedChatRoom?.id === room.id ? '#2f54eb' : '#f0f0f0'
                      }}
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        {room.unread_count > 0 && (
                          <Badge count={room.unread_count} style={{ backgroundColor: '#2f54eb' }} />
                        )}
                      </div>
                      ...
                      chatMessages.map(msg => (
                        ...
                        >
                          <div style={{
                            ...
                            padding: '10px 15px',
                            borderRadius: '15px',
                            background: msg.is_own ? '#2f54eb' : '#f0f0f0',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      {student.has_existing_chat ? (
                        <Tag color="geekblue">Existing Chat</Tag>
                      ) : (
        ...
        <TabPane tab={<span><DatabaseOutlined /> Cooperation Registry</span>} key="2">
            ...
            <Row gutter={16} align="middle">
              ...
              <Col span={8} style={{ textAlign: 'right' }}>
                <Space>
                  ...
                  <Button icon={<ReloadOutlined />} onClick={loadCoopRegistry} loading={loadingRegistry}>
                    Refresh
                  </Button>
                </Space>
              </Col>
          ...
          <Table
            ...
            title={() => (
              <Space>
                <DatabaseOutlined />
                <Text strong>Cooperation Sharing Registry</Text>
                <Tag color="geekblue">Total Records: {searchRegistry().length}</Tag>
                ...
                <Tag color="gold">
                  Total Amount: ${searchRegistry().reduce((sum, r) => sum + (r.amount || 0), 0)}
                </Tag>
              </Space>
            )}
          />
      ...
      <Modal
        title={
          <Space>
            <RocketOutlined style={{ color: '#2f54eb' }} />
        ...
        {batchProgress.status === 'idle' && (
          <>
            ...
            <Form form={batchForm} layout="vertical">
              ...
              {batchAction === 'reject' && (
                <>
                  <Divider>Obligation Requirements (Optional)</Divider>
                        ...
                        <Col span={12}>
                          ...
                          >
                            <Input
                              ...
                              placeholder="e.g., Cooperation fee"
                            />
                          </Form.Item>
                        </Col>
                      </Row>
              ...
              <div style={{ textAlign: 'center' }}>
                <Space size="large">
                  ...
                  <Button
                    ...
                    style={batchAction === 'approve' ? { background: '#2f54eb' } : {}}
                ...
                <List
                  ...
                  renderItem={item => (
                    <List.Item>
                      ...
                      {item.obligations > 0 && (
                        <Tag color="geekblue" style={{ marginLeft: 8 }}>Obligations: ${item.obligations}</Tag>
                      )}
                    </List.Item>
                  )}
                />
          ...
          <div style={{ maxHeight: '60vh', overflow: 'auto' }}>
            <Descriptions column={2} bordered size="middle" labelStyle={{ fontWeight: 'bold', background: '#fafafa' }}>
              ...
              <Descriptions.Item label="Reason" span={2}>
                <Card size="small" style={{ background: '#f0f5ff' }}>{selectedForm.reason}</Card>
              </Descriptions.Item>
              {selectedForm.campuspolice_note && (
                <Descriptions.Item label="Campus Police Note" span={2}>
                  <Alert message={selectedForm.campuspolice_note} type="info" showIcon />
                </Descriptions.Item>
              )}
            </Descriptions>

            {selectedForm.status === "approved_campuspolice" && (
      ...
      <Modal
        title={<Space><DatabaseOutlined /><span>{editingRecord ? 'Edit Cooperation Record' : 'Register New Cooperation Record'}</span></Space>}
        ...
        width={600}
      >
        <Alert message="Cooperation Sharing Registry" description="Keep track of student cooperation agreements and obligations." type="info" showIcon style={{ marginBottom: 20 }} />
        ...
        <Form form={form} layout="vertical" onFinish={editingRecord ? updateRecord : registerNewRecord}>
          ...
          <Form.Item name="cooperation_type" label="Cooperation Type" rules={[{ required: true }]}>
            <Select placeholder="Select cooperation type">
              <Option value="scholarship">Scholarship Agreement</Option>
              <Option value="exchange">Exchange Program</Option>
              <Option value="internship">Internship Agreement</Option>
              <Option value="research">Research Collaboration</Option>
              <Option value="funding">Funding Obligation</Option>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="amount" label="Amount">
                <InputNumber style={{ width: '100%' }} placeholder="Enter amount" prefix="$" min={0} step={0.01} />
              </Form.Item>
            </Col>
            <Col span={12}>
      ...
      <div style={{ marginTop: 40, padding: '20px 0', textAlign: 'center', borderTop: '1px solid #f0f0f0' }}>
        <Text type="secondary">
          Cooperation Sharing Clearance System • {new Date().getFullYear()} • 
          <span style={{ marginLeft: 8, color: '#2f54eb' }}>
            Total Forms: {stats.total} • Pending: {stats.pending} • Records: {coopRegistry.length}
          </span>
        </Text>
      </div>
    </div>
